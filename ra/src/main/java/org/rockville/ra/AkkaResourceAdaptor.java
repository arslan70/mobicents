...
public class AkkaResourceAdaptor implements ResourceAdaptor {
	 ...
	 * to a number of objects that are of interest to many Resource Adaptors. A
	 * resource adaptor object is provided with a ResourceAdaptorContext object
	 * when the setResourceAdaptorContext method of the ResourceAdaptor
	...
	/**
	 * A tracer is represented in the SLEE by the Tracer interface. Notification
	 * sources access the Tracer Facility through a Tracer object that
	 * implements the Tracer interface. A Tracer object can be obtained by SBBs
	 * via the SbbContext interface, by resource adaptor entities via the
	 ...
	 * Events.
	 */
	private Marshaler marshaler = new AkkaMarshaler();
	...
	public static final int UNREFERENCED_EVENT_FLAGS = EventFlags
			.setRequestEventReferenceReleasedCallback(DEFAULT_EVENT_FLAGS);
	...
	public void setResourceAdaptorContext(ResourceAdaptorContext context) {
		...
		this.tracer = context.getTracer("AkkaResourceAdaptor");
		...
		String path = config.getConfig("akka").getString("path");
		final ActorSelection remoteActorReference = system.actorSelection(path);
	}
	...
	public void raConfigure(ConfigProperties properties) {
		...
		if (tracer.isFineEnabled()) {
			tracer.fine("Akka Resource Adaptor configured.");
		}
	}
	...
	public void raUnconfigure() {
		...
		if (tracer.isFineEnabled()) {
			tracer.fine("Akka Resource Adaptor unconfigured.");
		}
	}
	...
	public void raActive() {
		...
		if (tracer.isFineEnabled()) {
			tracer.fine("Akka Resource Adaptor entity active.");
		}
	}
	...
	public void raStopping() {
		...
		if (tracer.isFineEnabled()) {
			tracer.fine("Akka Resource Adaptor entity stopping.");
		}
	}
	...
	public void raInactive() {
		...
		if (tracer.isFineEnabled()) {
			tracer.fine("Akka Resource Adaptor entity inactive.");
		}
	}
	...
	public void raVerifyConfiguration(ConfigProperties properties)
		...
		if (tracer.isFineEnabled()) {
			tracer.fine("Akka Resource Adaptor configuration verified.");
		}
	}
	...
	public Object getResourceAdaptorInterface(String className) {
		if ("org.rockville.ratype.AkkaProvider".equals(className)) {
			return new AkkaProviderImpl(this);
		}
	...
	public void send(String message){
		System.out.println("From RA:" + message);
		  ActorSelection selection = system.actorSelection("akka.tcp://PanicCallActorSystem@127.0.0.1:2550/user/pc-engine");
	        selection.tell(message, ActorRef.noSender());
	}
	// TODO: Add Akka Resource Adaptor specific logic.
}
